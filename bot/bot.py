...
HELP_GROUP_CHAT_MESSAGE = """Just talk to me! I am your assistant, girlfirnd and your friend...
"""
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Hi! I'm <b>EzCat</b> bot implemented with powerful LLMs 🤖\n\n"
...
async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    ...
    def check_image_in_prompt(message):
        unable_to_send_image = ['send or display images', 'unable to send']
        for utsi in unable_to_send_image:
            if utsi in message.lower():
                return True
        return False
    ...
    async def message_handle_fn():
        ...
        try:
            ...
            async for gen_item in gen:
                ...
                # if check_image_in_prompt(answer):
...
async def error_handle(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        for message_chunk in split_text_into_chunks(message, 4096):
            try:
                await context.bot.send_message(config.admin_user, message_chunk, parse_mode=ParseMode.HTML)
            except telegram.error.BadRequest:
                # answer has invalid characters, so we send it without parse_mode
                await context.bot.send_message(config.admin_user, message_chunk)
    except:
        await context.bot.send_message(config.admin_user, "Some error in error handler")
